...
def start_con():
    try:
        with tracer.start_as_current_span("listMovieData"):
            ...
            use_db_query = "USE mydb"
            create_table_query = "CREATE TABLE IF NOT EXISTS expenses (date VARCHAR(255), money VARCHAR(255))"
...
async def root(data = Body()):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("INSERT INTO expenses(date,money) VALUES (%s, %s)", (date,money))
